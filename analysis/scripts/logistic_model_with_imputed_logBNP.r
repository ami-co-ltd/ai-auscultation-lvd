...
# 3. glm 実行
regression_model <- glm(LVEF2_cat ~ age + sex + BMI + DPC_IHD + DPC_CM + Afib + log_BNP_pred, data = internal_data_raw, family = binomial)
...
write.csv(external_data, "logistic_output_external_with_logBNP.csv", row.names = FALSE)
...
# McFaddenの疑似決定係数
feature_cols <- c("log_BNP_pred", "age", "sex", "BMI", "DPC_IHD", "DPC_CM", "Afib")

# モデル全体の擬似R²
model_full <- glm(LVEF2_cat ~ log_BNP_pred + age + sex + BMI + DPC_IHD + DPC_CM + Afib,
