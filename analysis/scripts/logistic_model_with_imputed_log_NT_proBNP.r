...
# 3. glm 実行
regression_model <- glm(LVEF2_cat ~ age + sex + BMI + DPC_IHD + DPC_CM + Afib + log_NT_proBNP_pred, data = internal_data_raw, family = binomial)
...
write.csv(external_data, "logistic_output_external_with_imputed_log_NT_proBNP.csv", row.names = FALSE)
...
# McFaddenの疑似決定係数
feature_cols <- c("log_NT_proBNP_pred", "age", "sex", "BMI", "DPC_IHD", "DPC_CM", "Afib")

# モデル全体の擬似R²
model_full <- glm(LVEF2_cat ~ log_NT_proBNP_pred + age + sex + BMI + DPC_IHD + DPC_CM + Afib,
