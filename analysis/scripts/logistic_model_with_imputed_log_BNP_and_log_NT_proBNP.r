...
# 3. glm 実行
regression_model <- glm(LVEF2_cat ~ age + sex + BMI + DPC_IHD + DPC_CM + Afib + log_BNP_pred + log_NT_proBNP_pred, data = internal_data_raw, family = binomial)
...
write.csv(external_data, "logistic_output_external_with_imputed_log_BNP_and_log_NT_proBNP.csv", row.names = FALSE)

# 8. 性能評価
roc_reg_model_external <- roc(external_data$LVEF2_cat, external_data$pred_prob)
plot(roc_reg_model_external, print.auc =TRUE)

saveRDS(list(roc = roc_reg_model_external, label = "w/ imputed log BNP", n = nrow(external_data)), "roc_imputed_logBNP.rds")
...
# McFaddenの疑似決定係数
feature_cols <- c("log_BNP_pred", "log_NT_proBNP_pred", "age", "sex", "BMI", "DPC_IHD", "DPC_CM", "Afib")

# モデル全体の擬似R²
model_full <- glm(LVEF2_cat ~ log_BNP_pred + log_NT_proBNP_pred + age + sex + BMI + DPC_IHD + DPC_CM + Afib,
