...
# 3. glm 実行
regression_model <- glm(LVEF2_cat ~ age + sex + BMI + DPC_IHD + DPC_CM + Afib + pq_time + qrs_time, data = internal_with_feat, family = binomial)
...
write.csv(external_data, "logistic_output_external_with_PR_QRS.csv", row.names = FALSE)

# 8. 性能評価
roc_reg_model_external <- roc(external_data$LVEF2_cat, external_data$pred_prob)
plot(roc_reg_model_external, print.auc =TRUE)

saveRDS(list(roc = roc_reg_model_external, label = "w/ PQ time and QRS time", n = nrow(external_data)), "roc_pq_qrs.rds")
...
# McFaddenの疑似決定係数
feature_cols <- c("age", "sex", "BMI", "DPC_IHD", "DPC_CM", "Afib", "pq_time", "qrs_time")

# モデル全体の擬似R²
model_full <- glm(LVEF2_cat ~ age + sex + BMI + DPC_IHD + DPC_CM + Afib + pq_time + qrs_time,
